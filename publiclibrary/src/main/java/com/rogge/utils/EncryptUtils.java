...
import static com.rogge.utils.ConvertUtils.bytes2HexString;
import static com.rogge.utils.ConvertUtils.hexString2Bytes;
...
public class EncryptUtils {
    ...
    public static String encryptRSAForClient(String data, RSAPublicKey pubKey) {
        Cipher cipher;
        try {
            cipher = Cipher.getInstance("RSA/ECB/PKCS1Padding");
            cipher.init(Cipher.ENCRYPT_MODE, pubKey);
            int keyLength = pubKey.getModulus().bitLength() / 8;
            byte[] srcBits = data.getBytes("UTF-8");
            ByteBuffer inputBuffer = ByteBuffer.wrap(srcBits);
            ByteArrayOutputStream bos = new ByteArrayOutputStream();
            int pageSize = keyLength - 11;
            while (inputBuffer.hasRemaining()) {
                int readLength = inputBuffer.remaining() < pageSize ? inputBuffer.remaining() : pageSize;
                byte[] bits = new byte[readLength];
                inputBuffer.get(bits, 0, readLength);
                bos.write(cipher.doFinal(bits));
            }
            return EncodeUtils.base64Encode2String(bos.toByteArray());
        } catch (NoSuchAlgorithmException e) {
            e.printStackTrace();
    ...
    public static RSAPublicKey getPubKey(String pubKey) {
        ...
        try {
            ...
            X509EncodedKeySpec keySpec = new X509EncodedKeySpec(EncodeUtils.base64Decode(pubKey));
    ...
    public static String aesEncrypt(String src, String key) {
        ...
        try {
            ...
            return EncodeUtils.base64Encode2String(cipher.doFinal(src.getBytes("UTF-8")));
    ...
    public static String aesDecrypt(String src, String key) {
        ...
        try {
            ...
            return new String(cipher.doFinal(EncodeUtils.base64Decode(src)), "UTF-8");
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        } catch (NoSuchAlgorithmException e) {
            e.printStackTrace();
    ...
    public static String bitToAsc(byte[] bits) {
